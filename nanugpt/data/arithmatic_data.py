...
def _process_n_digits(n_digits: int, max_samples: int, context_len: int,
...
def get_data(device_batch_size: int, eval_batch_size:int, data_loader_seed:int,
             min_digits: int, max_digits: int, max_samples: int, context_len: int)->Tuple[DataLoader,DataLoader, Optional[DataLoader]]:
    ...
    train_strings, val_strings = gen_mul_seqs(min_digits, max_digits, max_samples, context_len,
                           op='*', eq='=',
                           sep=tokenizer.eos_str, pad=tokenizer.pad_str,
                           seed=seed)
    train_tensors = tokenizer.strings2tensor(train_strings)
    val_tensors = tokenizer.strings2tensor(val_strings)
