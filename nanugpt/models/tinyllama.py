...
class LlamaConfig:
    ...
    def norm_class(self) -> Type:
        ...
        if self._norm_class == "RMSNorm":
            ...
            from nanugpt.models.rmsnorm import FusedRMSNorm
            return FusedRMSNorm
...
class Llama(nn.Module):
    ...
    def build_kv_caches(self, idx: torch.Tensor, max_seq_length: int, rope_cache_length: int) -> List[KVCache]:
        ...
        device = idx.device
        return [
            (torch.zeros(k_cache_shape, device=device), torch.zeros(v_cache_shape, device=device)) # type: ignore
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config: LlamaConfig) -> None:
        super().__init__()
        shape = (config.n_head + 2 * config.n_query_groups) * config.head_size # type: ignore
    ...
    def forward(
        ...
        q_per_kv = self.config.n_head // self.config.n_query_groups # type: ignore
        ...
        if kv_cache is not None:
            ...
            cache_k, cache_v = cache_k.to(dtype=k.dtype), cache_v.to(dtype=v.dtype) # type: ignore
            # check if reached token limit
            if input_pos[-1] >= max_seq_length: # type: ignore
                input_pos = torch.tensor(max_seq_length - 1, device=input_pos.device) # type: ignore
                # shift 1 position to the left
                cache_k = torch.roll(cache_k, -1, dims=1)
                cache_v = torch.roll(cache_v, -1, dims=1)

            k = cache_k.index_copy_(1, input_pos, k) # type: ignore
            v = cache_v.index_copy_(1, input_pos, v) # type: ignore
            kv_cache = k, v

        y = self.scaled_dot_product_attention(q, k, v, mask=mask) # type: ignore
...
class GptNeoxMLP(nn.Module):
    def __init__(self, config: LlamaConfig) -> None:
        super().__init__()
        self.fc = nn.Linear(config.n_embd, config.intermediate_size, bias=config.bias) # type: ignore
