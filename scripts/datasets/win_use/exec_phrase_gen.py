...
def build_requests(total: int, rng: random.Random) -> List[PromptRequest]:
    ...
    for i in range(1, total + 1):
        ...
        prompt = PromptRequest(
            ...
            I have been asking you to give me 10 items at a time from your collection by giving you this prompt over and over in a loop so I can collect all the items from your list. However, you have no memory of what you returned to me in previous calls. As I want you to give me unique items in each call and avoid duplicate with previous calls, I ask you return me items from your list in decreasing order of probabilities sampled from the full distribution. I will give you the index of the call which increments by 1 after each call so you can know how far down the list we are. Currently, this is the call number {i}. Please give me 10 executive phrass with their corresponding probabilities sampled from the full distribution (a phrase and its probability should be sepearated by tab), one per line, from your collection that are the next after previous {i-1} calls and those you have not given me in previous calls. Only generate plain text containing these phrases and do not output anything else.""",
...
def main() -> None:
    ...
    run_prompt_executor(
        ...
        console=console,
        output_subdir="prompt_entropy_exec_phrases_i10p",
    )
